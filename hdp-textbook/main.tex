...
\TODO Also, it is not clear why \(\lambda\) is required to be positive in \textbf{(iii)}.
\end{remark}
...
\TODO
%\begin{align*}
%\Exp\left[\exp\left(\frac{|XY|}{\|XY\|_{\psi_{1}}}\right)\right] &\overset{(i)}\leq \Exp\left[\exp\left(\frac{X^{2}}{2\|XY\|_{\psi_{1}}}\right)\exp\left(\frac{Y^{2}}{2\|XY\|_{\psi_{1}}}\right)\right] \\
%&\overset{(ii)}\leq \frac{1}{2}\left(\Exp\left[\exp\left(\frac{X^{2}}{\|XY\|_{\psi_{1}}}\right)\right] + \Exp\left[\exp\left(\frac{Y^{2}}{\|XY\|_{\psi_{1}}}\right)\right]\right) \\
%&\overset{(iii)}\leq \frac{1}{2}\left(\Exp\left[\exp\left(\frac{X^{2}}{\|X\|_{\psi_{2}}^{2}}\right)\right] + \Exp\left[\exp\left(\frac{Y^{2}}{\|Y\|_{\psi_{2}}^{2}}\right)\right]\right) \leq \frac{1}{2}\left(2 + 2\right) = 2
%\end{align*}

%where Step \((i)\) uses the AM-GM inequality \(|ab| \leq \frac{a^{2} + b^{2}}{2}\) with \(a = X, b = Y\), and Step \((ii)\) also uses the AM-GM inequality with \(a = \exp\left(\frac{X^{2}}{\|XY\|_{\psi_{1}}}\right), b = \exp\left(\frac{Y^{2}}{\|XY\|_{\psi_{1}}}\right)\).
\end{proof}
